...
브라우저 샌드박스가 막는 것과 못 막는 것을 갈라 보면 이 선택의 이유가 분명하다.

| 막힌다 | 안 막힌다 |
|---|---|
| 파일 시스템 접근 | 무한 루프로 탭 멈추기 |
| 다른 출처로의 요청 (CORS) | `fetch` 로 자기 서버에 데이터 보내기 |
| 다른 탭의 내용 | 이 출처의 `localStorage` 읽기 |

오른쪽 칸이 비어 있지 않으므로 "샌드박스가 있으니 괜찮다"로 끝낼 수 없다.
그렇다고 Web Worker 로 격리하는 것은 (무한 루프를 죽일 수 있고 DOM 접근이
막힌다) Pyodide 를 Worker 로 옮기는 구조 변경이라 비용이 크다. 사람이 읽고
누르는 것이 그 사이에서 가장 값싸고 정직한 답이다.
