...
span 을 훑으므로 evaluate 는 `O(연산 × span)` 이고, span 은 연산에 비례해
자라므로 전체가 사실상 `O(n²)` 이다. 시간 배분도 한쪽으로 몰려 있다.
...
2. **`turned()` 짝 접합** — 회전과 그 역회전이 짝으로 도는데 살아남는 것은
   그 사이 split 이 추가한 호뿐이다. 전체 시간의 절반이 버려지는 상태 변경에
   들어간다. OctoCube 계열 정의의 골격이라 이득이 크다
3. **carrier별 `m`, `s` 캐시**
...
1. **실시간 최적화** — `turned()` 짝 접합, 구성 끝 snapshot, trace + guard
   (§12.3). numpy 제거로 4~7배를 벌었으나 slider 는 아직 매번 처음부터
   재평가한다. 접두부 snapshot 만으로는 이 부분이 해결되지 않는다 (§13.1)
...
**성능이 선행 조건이다.** §12.2 로 CPython 기준 4~7배를 벌었다. 환산하면:

| 정의 | 연산 | 제거 전 | 제거 후 | 브라우저 추정 |
|---|---|---|---|---|
| quantum | 37 | 0.008s | 0.003s | 괜찮음 |
| octododeca | 97 | 0.132s | 0.020s | 0.05~0.1s. 쓸 만함 |
| 합성 k=10 | 367 | 1.58s | 0.35s | 0.7~1.8s. slider 가 뻑뻑함 |

몸통 정의가 돌긴 하지만 slider 실시간은 아직 아니다. §12.3 의 2번(`turned()`
짝 접합)까지 하고 다시 잰다. 접두부 snapshot 은 여기에 도움이 되지 않는다
(§13.1).
