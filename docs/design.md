...
examples/               퍼즐 정의
web/                    Canvas 2D 프로토타입 (§11.2). index.html + render.js
tests/
```
...
**`exec` 살포.** 링크 공유를 하기로 했으므로 정면으로 다뤄야 한다. 코드는 URL
**fragment**(`#`)에 싣는다 — fragment 는 서버로 전송되지 않아 호스팅 쪽에
로그가 남지 않는다. 그래도 공유받은 링크를 여는 것은 임의 코드 실행이다.
브라우저 샌드박스가 막아 주는 범위와 막지 못하는 범위를 편집창을 만들 때
정한다.
