...
1. **실시간 최적화** — 구성 끝 snapshot, carrier `m`/`s` 캐시, trace + guard
   (§12.3). numpy 제거와 접합으로 몸통 정의가 밀리초대로 내려왔다. slider 는
   아직 매번 처음부터 재평가하지만, 이제 그 비용이 감당할 크기다.
   접두부 snapshot 만으로는 slider 가 해결되지 않는다 (§13.1)
