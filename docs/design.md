...
1. **registry 를 거치지 않는다.** 마커는 표시 전용이다. 마커 원이
   ...
   종류다. `PuzzleFamily` 에서 직접 만들고 `evaluate` 를 태우지 않는다
...
1. **`fetch` 구멍 막기** — CSP `connect-src` 로 잠근다. Pyodide 를 CDN 에서
   받는 한 그 출처를 열어 두어야 하므로 자체 호스팅이 전제다 (§19.5).
   지금은 페이지가 들고 있는 것이 없어 훔칠 것이 없다 — **그것이 지켜야 할
   성질이다.** 무언가를 저장하게 되면 그때 이 구멍이 실제 위험이 된다
2. **GitHub Pages 실제 배포** — 저장소를 그대로 서빙하면 되는 상태지만 (§19.3)
   설정과 URL 확인을 안 했다
...
축 id 는 이미 약자 + 숫자다 — `c0..c5`, `rd0..rd11`, `ph0..ph59`. 메뉴가 만드는
변수도 같은 규약을 쓴다.

```python
c  = cube("cube")
rd = rhombic_dodecahedron("rhombic_dodecahedron")
i  = icosahedron("icosa")
```

집합 id 는 프리셋이 정해 둔 짧은 이름(`cube`, `icosa`)이다. 슬라이더에 그대로
나오므로 변수보다 설명적이어야 한다.

**같은 입체를 두 번 넣으면 축 id 가 겹친다.** 접두사가 입체마다 고정이라
`c0` 이 두 집합에 생기는데, 축 id 는 전 집합에서 유일해야 한다 (§5). 그래서
프리셋 factory 가 `prefix` 를 받도록 열고, 두 번째부터는 달리 준다.

```python
c2 = cube("cube2", prefix="c2")
```
