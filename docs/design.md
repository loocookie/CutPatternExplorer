...
`pyodide.mjs` 를 `cdn.jsdelivr.net` 에서 받는다. Pages 에 얹지 않는 이유는
크기와 갱신이지만, **정의가 `fetch` 를 쓸 수 있는 구멍**(§19.4)이 여기 걸린다 —
자체 호스팅으로 바꾸고 `connect-src` 를 잠그는 것이 짝이다. 배포와 분리한다:
CSP 를 잘못 걸면 페이지가 조용히 안 뜨는데, 그 확인은 브라우저에서만 된다.
