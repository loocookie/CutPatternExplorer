...
`plan_conjugation` 이 연산 목록을 훑어 짝을 찾는다. 다음은 거부한다.
...
범위를 나중에 안전하게 넓힐 수 있다.

아직 안 잡는 것: 맨 `turn()` 과 정의 끝의 자동 `RollbackTurns` 로 이루어지는
짝. 대수는 같게 성립하지만(`E ∪ Φa⁻¹(C₁) ∪ (Φb∘Φa)⁻¹(C₂)`) 짝이 어휘적으로
보이지 않아 스택 추적이 필요하다.
...
2. ~~**`turned()` 짝 접합**~~ — **완료** (§7.10). `E ∪ Φ⁻¹(C)` 로 낮춰 회전을
   ...
   맨 `turn()` + 자동 `RollbackTurns` 짝은 아직 폴백이다 (§7.10)
3. **carrier별 `m`, `s` 캐시**
...
- 증명 못 하는 것(맨 turn, 짝 사이의 region 블록)을 거부하는가
- 블록 바깥의 영역은 접합하는가
