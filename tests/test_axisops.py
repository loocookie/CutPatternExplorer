...
def test_merge_dedupes_identical_directions():
    """같은 방향은 먼저 온 쪽만 남는다."""
    assert len(merge("dup", S.cube(), S.cube())) == 6


def test_merge_of_cube_and_octahedron_is_a_cuboctahedron_axis_system():
    """아르키메데스 다면체 대부분은 방향집합이 플라톤의 합집합이다."""
    m = merge("cubocta", S.cube(), S.octahedron())
    assert signature(m) == signature(merge("x", S.octahedron(), S.cube()))


def test_merge_with_a_rotated_copy():
    rotated = rotate(S.cube(), axis=(1, 1, 1), angle=180)
    both = merge("pair", S.cube(), rotated)
...
def test_rotate_by_quaternion_matches_axis_angle():
    ...
    q = rotate(S.cube(), quaternion=(math.cos(half), 0, 0, math.sin(half)))
    a = rotate(S.cube(), axis=(0, 0, 1), angle=90)
    for x, y in zip(q, a):
...
def test_tetrahedron_is_not_centrally_symmetric():
    ...
    assert len(both) == 8
    assert signature(both) == signature(S.octahedron())


def test_invert_preserves_pairwise_angles():
    assert signature(invert(S.icosahedron())) == signature(S.icosahedron())
